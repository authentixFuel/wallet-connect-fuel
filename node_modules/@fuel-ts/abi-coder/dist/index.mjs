...
import { toBytes, bn } from "@fuel-ts/math";
var encodedLengths = {
  u64: WORD_SIZE,
  u256: WORD_SIZE * 4
};
...
var ArrayCoder = class extends Coder {
  ...
  encode(value) {
    if (!Array.isArray(value)) {
      throw new FuelError3(ErrorCode3.ENCODE_ERROR, `Expected array value.`);
    }
    if (this.length !== value.length) {
      throw new FuelError3(ErrorCode3.ENCODE_ERROR, `Types/values length mismatch.`);
    }
  ...
  decode(data, offset) {
    if (data.length < this.encodedLength || data.length > MAX_BYTES) {
      throw new FuelError3(ErrorCode3.DECODE_ERROR, `Invalid array data size.`);
    }
...
var B256Coder = class extends Coder {
  ...
  decode(data, offset) {
    if (data.length < this.encodedLength) {
      throw new FuelError4(ErrorCode4.DECODE_ERROR, `Invalid b256 data size.`);
    }
    let bytes = data.slice(offset, offset + this.encodedLength);
    ...
    if (bytes.length !== this.encodedLength) {
      throw new FuelError4(ErrorCode4.DECODE_ERROR, `Invalid b256 byte data size.`);
    }
...
var B512Coder = class extends Coder {
  ...
  encode(value) {
    let encodedValue;
    try {
      encodedValue = arrayify3(value);
    } catch (error) {
      throw new FuelError5(ErrorCode5.ENCODE_ERROR, `Invalid ${this.type}.`);
    }
    if (encodedValue.length !== this.encodedLength) {
      throw new FuelError5(ErrorCode5.ENCODE_ERROR, `Invalid ${this.type}.`);
    }
  ...
  decode(data, offset) {
    if (data.length < this.encodedLength) {
      throw new FuelError5(ErrorCode5.DECODE_ERROR, `Invalid b512 data size.`);
    }
    let bytes = data.slice(offset, offset + this.encodedLength);
    ...
    if (bytes.length !== this.encodedLength) {
      throw new FuelError5(ErrorCode5.DECODE_ERROR, `Invalid b512 byte data size.`);
    }
...
var BooleanCoder = class extends Coder {
  ...
  encode(value) {
    ...
    if (!isTrueBool) {
      throw new FuelError6(ErrorCode6.ENCODE_ERROR, `Invalid boolean value.`);
    }
    const output = toBytes2(value ? 1 : 0, this.paddingLength);
  ...
  decode(data, offset) {
    if (data.length < this.paddingLength) {
      throw new FuelError6(ErrorCode6.DECODE_ERROR, `Invalid boolean data size.`);
    }
    let bytes;
    if (this.options.isRightPadded) {
    ...
    const decodedValue = bn4(bytes);
    if (decodedValue.isZero()) {
      return [false, offset + this.paddingLength];
    }
    if (!decodedValue.eq(bn4(1))) {
      throw new FuelError6(ErrorCode6.DECODE_ERROR, `Invalid boolean value.`);
    }
...
var ByteCoder = class extends Coder {
  ...
  encode(value) {
    if (!Array.isArray(value)) {
      throw new FuelError7(ErrorCode7.ENCODE_ERROR, `Expected array value.`);
    }
    const parts = [];
  ...
  decode(data, offset) {
    if (data.length < BASE_VECTOR_OFFSET) {
      throw new FuelError7(ErrorCode7.DECODE_ERROR, `Invalid byte data size.`);
    }
    const len = data.slice(16, 24);
    const encodedLength = bn5(new BigNumberCoder("u64").decode(len, 0)[0]).toNumber();
    const byteData = data.slice(BASE_VECTOR_OFFSET, BASE_VECTOR_OFFSET + encodedLength);
    if (byteData.length !== encodedLength) {
      throw new FuelError7(ErrorCode7.DECODE_ERROR, `Invalid bytes byte data size.`);
    }
...
var EnumCoder = class extends Coder {
  ...
  constructor(name, coders) {
    ...
    return concat3([this.#caseIndexCoder.encode(caseIndex), padding, encodedValue]);
  }
  encode(value) {
    ...
    if (!caseKey) {
      throw new FuelError8(ErrorCode8.INVALID_DECODE_VALUE, "A field for the case must be provided.");
    }
    if (empty.length !== 0) {
      throw new FuelError8(ErrorCode8.INVALID_DECODE_VALUE, "Only one field must be provided.");
    }
  ...
  decode(data, offset) {
    if (data.length < this.#encodedValueSize) {
      throw new FuelError8(ErrorCode8.DECODE_ERROR, `Invalid enum data size.`);
    }
    let newOffset = offset;
    let decoded;
    ...
    if (!caseKey) {
      throw new FuelError8(
        ErrorCode8.INVALID_DECODE_VALUE,
...
var OptionCoder = class extends EnumCoder {
  ...
  decode(data, offset) {
    if (data.length < this.encodedLength - 1) {
      throw new FuelError9(ErrorCode9.DECODE_ERROR, `Invalid option data size.`);
    }
...
var NumberCoder = class extends Coder {
  ...
  encode(value) {
    ...
    try {
      ...
      throw new FuelError10(ErrorCode10.ENCODE_ERROR, `Invalid ${this.baseType}.`);
    }
    if (bytes.length > this.length) {
      throw new FuelError10(ErrorCode10.ENCODE_ERROR, `Invalid ${this.baseType}, too many bytes.`);
    }
    const output = toBytes3(bytes, this.paddingLength);
    if (this.baseType !== "u8") {
      return output;
    }
  ...
  decodeU8(data, offset) {
    ...
    return [toNumber2(bytes), offset + this.paddingLength];
  }
  decode(data, offset) {
    if (data.length < this.paddingLength) {
      throw new FuelError10(ErrorCode10.DECODE_ERROR, `Invalid number data size.`);
    }
    if (this.baseType === "u8") {
    ...
    if (bytes.length !== this.paddingLength - (this.paddingLength - this.length)) {
      throw new FuelError10(ErrorCode10.DECODE_ERROR, `Invalid number byte data size.`);
    }
...
var RawSliceCoder = class extends Coder {
  ...
  encode(value) {
    if (!Array.isArray(value)) {
      throw new FuelError11(ErrorCode11.ENCODE_ERROR, `Expected array value.`);
    }
    const parts = [];
...
var StdStringCoder = class extends Coder {
  ...
  decode(data, offset) {
    if (data.length < this.encodedLength) {
      throw new FuelError12(ErrorCode12.DECODE_ERROR, `Invalid std string data size.`);
    }
    const len = data.slice(16, 24);
    const encodedLength = bn6(new BigNumberCoder("u64").decode(len, 0)[0]).toNumber();
    const byteData = data.slice(BASE_VECTOR_OFFSET, BASE_VECTOR_OFFSET + encodedLength);
    if (byteData.length !== encodedLength) {
      throw new FuelError12(ErrorCode12.DECODE_ERROR, `Invalid std string byte data size.`);
    }
...
var StringCoder = class extends Coder {
  ...
  encode(value) {
    if (this.length !== value.length) {
      throw new FuelError13(ErrorCode13.ENCODE_ERROR, `Value length mismatch during encode.`);
    }
    const encoded = toUtf8Bytes2(value);
    const padding = new Uint8Array(this.#paddingLength);
    return concat5([encoded, padding]);
  }
  decode(data, offset) {
    if (data.length < this.encodedLength) {
      throw new FuelError13(ErrorCode13.DECODE_ERROR, `Invalid string data size.`);
    }
    const bytes = data.slice(offset, offset + this.length);
    if (bytes.length !== this.length) {
      throw new FuelError13(ErrorCode13.DECODE_ERROR, `Invalid string byte data size.`);
    }
    const value = toUtf8String2(bytes);
...
var StructCoder = class extends Coder {
  ...
  decode(data, offset) {
    if (data.length < this.encodedLength) {
      throw new FuelError14(ErrorCode14.DECODE_ERROR, `Invalid struct data size.`);
    }
    let newOffset = offset;
...
var TupleCoder = class extends Coder {
  ...
  encode(value) {
    if (this.coders.length !== value.length) {
      throw new FuelError15(ErrorCode15.ENCODE_ERROR, `Types/values length mismatch.`);
    }
  ...
  decode(data, offset) {
    if (data.length < this.encodedLength) {
      throw new FuelError15(ErrorCode15.DECODE_ERROR, `Invalid tuple data size.`);
    }
    let newOffset = offset;
...
var VecCoder = class extends Coder {
  ...
  encode(value) {
    if (!Array.isArray(value)) {
      throw new FuelError16(ErrorCode16.ENCODE_ERROR, `Expected array value.`);
    }
    const parts = [];
    const pointer = new BigNumberCoder("u64").encode(BASE_VECTOR_OFFSET);
    pointer.dynamicData = {
  ...
  decode(data, offset) {
    if (data.length < BASE_VECTOR_OFFSET || data.length > MAX_BYTES) {
      throw new FuelError16(ErrorCode16.DECODE_ERROR, `Invalid vec data size.`);
    }
    const len = data.slice(16, 24);
    const encodedLength = bn7(new BigNumberCoder("u64").decode(len, 0)[0]).toNumber();
    ...
    if (vectorRawData.length !== vectorRawDataLength) {
      throw new FuelError16(ErrorCode16.DECODE_ERROR, `Invalid vec byte data size.`);
    }
    return [
...
// src/Interface.ts
import { ErrorCode as ErrorCode32, FuelError as FuelError32 } from "@fuel-ts/errors";
import { arrayify as arrayify5 } from "@fuel-ts/utils";
...
import { ErrorCode as ErrorCode30, FuelError as FuelError30 } from "@fuel-ts/errors";
...
import { ErrorCode as ErrorCode29, FuelError as FuelError29 } from "@fuel-ts/errors";

// src/encoding/coders/v1/BooleanCoder.ts
import { ErrorCode as ErrorCode19, FuelError as FuelError19 } from "@fuel-ts/errors";
import { bn as bn8, toBytes as toBytes4 } from "@fuel-ts/math";
var BooleanCoder2 = class extends Coder {
  ...
  encode(value) {
    const isTrueBool = value === true || value === false;
    if (!isTrueBool) {
      throw new FuelError19(ErrorCode19.ENCODE_ERROR, `Invalid boolean value.`);
    }
    return toBytes4(value ? 1 : 0, this.encodedLength);
  }
  decode(data, offset) {
    if (data.length < this.encodedLength) {
      throw new FuelError19(ErrorCode19.DECODE_ERROR, `Invalid boolean data size.`);
    }
    const bytes = bn8(data.slice(offset, offset + this.encodedLength));
    if (bytes.isZero()) {
      return [false, offset + this.encodedLength];
    }
    if (!bytes.eq(bn8(1))) {
      throw new FuelError19(ErrorCode19.DECODE_ERROR, `Invalid boolean value.`);
    }
...
var ByteCoder2 = class extends Coder {
  ...
  encode(value) {
    if (!Array.isArray(value)) {
      throw new FuelError20(ErrorCode20.ENCODE_ERROR, `Expected array value.`);
    }
  ...
  decode(data, offset) {
    if (data.length < WORD_SIZE) {
      throw new FuelError20(ErrorCode20.DECODE_ERROR, `Invalid byte data size.`);
    }
    ...
    if (dataBytes.length !== length) {
      throw new FuelError20(ErrorCode20.DECODE_ERROR, `Invalid bytes byte data size.`);
    }
...
import { toNumber as toNumber3 } from "@fuel-ts/math";
...
var EnumCoder2 = class extends Coder {
  ...
  constructor(name, coders) {
    ...
    const caseIndex = Object.keys(this.coders).indexOf(value);
    const padding = new Uint8Array(this.#encodedValueSize - valueCoder.encodedLength);
    return concat6([this.#caseIndexCoder.encode(caseIndex), padding, encodedValue]);
  }
  encode(value) {
    if (typeof value === "string" && this.coders[value]) {
      return this.#encodeNativeEnum(value);
    ...
    const [caseKey, ...empty] = Object.keys(value);
    if (!caseKey) {
      throw new FuelError21(ErrorCode21.INVALID_DECODE_VALUE, "A field for the case must be provided.");
    }
    if (empty.length !== 0) {
      throw new FuelError21(ErrorCode21.INVALID_DECODE_VALUE, "Only one field must be provided.");
    }
    const valueCoder = this.coders[caseKey];
    const caseIndex = Object.keys(this.coders).indexOf(caseKey);
    const encodedValue = valueCoder.encode(value[caseKey]);
  ...
  decode(data, offset) {
    if (data.length < this.#encodedValueSize) {
      throw new FuelError21(ErrorCode21.DECODE_ERROR, `Invalid enum data size.`);
    }
    const caseBytes = new BigNumberCoder("u64").decode(data, offset)[0];
    const caseIndex = toNumber3(caseBytes);
    const caseKey = Object.keys(this.coders)[caseIndex];
    if (!caseKey) {
      throw new FuelError21(
        ErrorCode21.INVALID_DECODE_VALUE,
        `Invalid caseIndex "${caseIndex}". Valid cases: ${Object.keys(this.coders)}.`
      );
    }
    const valueCoder = this.coders[caseKey];
...
import { toNumber as toNumber4, toBytes as toBytes5 } from "@fuel-ts/math";
var getLength = (baseType) => {
  switch (baseType) {
    ...
    case "u32":
      return 4;
    default:
      throw new FuelError22(ErrorCode22.TYPE_NOT_SUPPORTED, `Invalid number type: ${baseType}`);
  }
};
var NumberCoder2 = class extends Coder {
  ...
  encode(value) {
    let bytes;
    try {
      bytes = toBytes5(value);
    } catch (error) {
      throw new FuelError22(ErrorCode22.ENCODE_ERROR, `Invalid ${this.baseType}.`);
    }
    if (bytes.length > this.length) {
      throw new FuelError22(ErrorCode22.ENCODE_ERROR, `Invalid ${this.baseType}, too many bytes.`);
    }
    return toBytes5(bytes, this.length);
  }
  decode(data, offset) {
    if (data.length < this.encodedLength) {
      throw new FuelError22(ErrorCode22.DECODE_ERROR, `Invalid number data size.`);
    }
    const bytes = data.slice(offset, offset + this.length);
    if (bytes.length !== this.encodedLength) {
      throw new FuelError22(ErrorCode22.DECODE_ERROR, `Invalid number byte data size.`);
    }
    return [toNumber4(bytes), offset + this.length];
  }
};
...
var RawSliceCoder2 = class extends Coder {
  ...
  encode(value) {
    if (!Array.isArray(value)) {
      throw new FuelError23(ErrorCode23.ENCODE_ERROR, `Expected array value.`);
    }
  ...
  decode(data, offset) {
    if (data.length < this.encodedLength) {
      throw new FuelError23(ErrorCode23.DECODE_ERROR, `Invalid raw slice data size.`);
    }
    const offsetAndLength = offset + WORD_SIZE;
    const lengthBytes = data.slice(offset, offsetAndLength);
    const length = bn10(new BigNumberCoder("u64").decode(lengthBytes, 0)[0]).toNumber();
    const dataBytes = data.slice(offsetAndLength, offsetAndLength + length);
    if (dataBytes.length !== length) {
      throw new FuelError23(ErrorCode23.DECODE_ERROR, `Invalid raw slice byte data size.`);
    }
...
import { toUtf8Bytes as toUtf8Bytes3, toUtf8String as toUtf8String3 } from "ethers";
var StdStringCoder2 = class extends Coder {
  constructor() {
    super("struct", "struct String", WORD_SIZE);
  }
  encode(value) {
    const bytes = toUtf8Bytes3(value);
    const lengthBytes = new BigNumberCoder("u64").encode(value.length);
    return new Uint8Array([...lengthBytes, ...bytes]);
  }
  decode(data, offset) {
    if (data.length < this.encodedLength) {
      throw new FuelError24(ErrorCode24.DECODE_ERROR, `Invalid std string data size.`);
    }
    const offsetAndLength = offset + WORD_SIZE;
    const lengthBytes = data.slice(offset, offsetAndLength);
    const length = bn11(new BigNumberCoder("u64").decode(lengthBytes, 0)[0]).toNumber();
    const dataBytes = data.slice(offsetAndLength, offsetAndLength + length);
    if (dataBytes.length !== length) {
      throw new FuelError24(ErrorCode24.DECODE_ERROR, `Invalid std string byte data size.`);
    }
    return [toUtf8String3(dataBytes), offsetAndLength + length];
  }
};
...
import { toUtf8Bytes as toUtf8Bytes4, toUtf8String as toUtf8String4 } from "ethers";
var StringCoder2 = class extends Coder {
  ...
  encode(value) {
    if (value.length !== this.encodedLength) {
      throw new FuelError25(ErrorCode25.ENCODE_ERROR, `Value length mismatch during encode.`);
    }
    return toUtf8Bytes4(value);
  }
  decode(data, offset) {
    if (data.length < this.encodedLength) {
      throw new FuelError25(ErrorCode25.DECODE_ERROR, `Invalid string data size.`);
    }
    const bytes = data.slice(offset, offset + this.encodedLength);
    if (bytes.length !== this.encodedLength) {
      throw new FuelError25(ErrorCode25.DECODE_ERROR, `Invalid string byte data size.`);
    }
    return [toUtf8String4(bytes), offset + this.encodedLength];
  }
};

// src/encoding/coders/v1/StructCoder.ts
import { ErrorCode as ErrorCode26, FuelError as FuelError26 } from "@fuel-ts/errors";
...
var StructCoder2 = class extends Coder {
  ...
  decode(data, offset) {
    if (data.length < this.encodedLength) {
      throw new FuelError26(ErrorCode26.DECODE_ERROR, `Invalid struct data size.`);
    }
    let newOffset = offset;
    const decodedValue = Object.keys(this.coders).reduce((obj, fieldName) => {
      const fieldCoder = this.coders[fieldName];
...
import { ErrorCode as ErrorCode27, FuelError as FuelError27 } from "@fuel-ts/errors";
...
var TupleCoder2 = class extends Coder {
  ...
  encode(value) {
    if (this.coders.length !== value.length) {
      throw new FuelError27(ErrorCode27.ENCODE_ERROR, `Types/values length mismatch.`);
    }
    return concatBytes3(this.coders.map((coder, i) => coder.encode(value[i])));
  }
  decode(data, offset) {
    if (data.length < this.encodedLength) {
      throw new FuelError27(ErrorCode27.DECODE_ERROR, `Invalid tuple data size.`);
    }
    let newOffset = offset;
    const decodedValue = this.coders.map((coder) => {
      let decoded;
...
import { ErrorCode as ErrorCode28, FuelError as FuelError28 } from "@fuel-ts/errors";
import { bn as bn12 } from "@fuel-ts/math";
import { concatBytes as concatBytes4 } from "@fuel-ts/utils";
var VecCoder2 = class extends Coder {
  ...
  encode(value) {
    if (!Array.isArray(value)) {
      throw new FuelError28(ErrorCode28.ENCODE_ERROR, `Expected array value.`);
    }
    const bytes = value.map((v) => this.coder.encode(v));
    const lengthBytes = new BigNumberCoder("u64").encode(value.length);
    return new Uint8Array([...lengthBytes, ...concatBytes4(bytes)]);
  }
  decode(data, offset) {
    if (data.length < this.encodedLength || data.length > MAX_BYTES) {
      throw new FuelError28(ErrorCode28.DECODE_ERROR, `Invalid vec data size.`);
    }
    const offsetAndLength = offset + WORD_SIZE;
    const lengthBytes = data.slice(offset, offsetAndLength);
    const length = bn12(new BigNumberCoder("u64").decode(lengthBytes, 0)[0]).toNumber();
    ...
    if (dataBytes.length !== dataLength) {
      throw new FuelError28(ErrorCode28.DECODE_ERROR, `Invalid vec byte data size.`);
    }
    return [
...
var getCoder2 = (resolvedAbiType, _options) => {
  ...
  if (resolvedAbiType.type === STR_SLICE_CODER_TYPE) {
    throw new FuelError29(
      ErrorCode29.INVALID_DATA,
      "String slices can not be decoded from logs. Convert the slice to `str[N]` with `__to_str_array`"
    );
  }
  throw new FuelError29(
    ErrorCode29.CODER_NOT_FOUND,
    `Coder not found: ${JSON.stringify(resolvedAbiType)}.`
  );
};
  ...
  switch (encoding) {
    ...
    default:
      throw new FuelError30(
        ErrorCode30.UNSUPPORTED_ENCODING_VERSION,
...
import { ErrorCode as ErrorCode31, FuelError as FuelError31 } from "@fuel-ts/errors";
...
var FunctionFragment = class {
  ...
  static getFunctionSelector(functionSignature) {
    ...
    return bn13(hashedFunctionSignature.slice(0, 10)).toHex(8);
  }
  #isInputDataPointer() {
  ...
  static verifyArgsAndInputsAlign(args, inputs, abi) {
    ...
    throw new FuelError31(ErrorCode31.ABI_TYPES_AND_VALUES_MISMATCH, errorMsg);
  }
...
var Interface = class {
  ...
  getFunction(nameOrSignatureOrSelector) {
    ...
    throw new FuelError32(
      ErrorCode32.FUNCTION_NOT_FOUND,
  ...
  decodeFunctionData(functionFragment, data) {
    ...
    if (!fragment) {
      throw new FuelError32(ErrorCode32.FRAGMENT_NOT_FOUND, "Fragment not found.");
    }
  ...
  decodeLog(data, logId) {
    ...
    return AbiCoder.decode(this.jsonAbi, loggedType, arrayify5(data), 0, {
  ...
  encodeConfigurable(name, value) {
    const configurable = findOrThrow(
      ...
      () => {
        throw new FuelError32(
          ErrorCode32.CONFIGURABLE_NOT_FOUND,

...
var scriptResultDecoder = (contractId, isOutputDataHeap) => (result) => {
  ...
  return receipts.filter(({ type }) => isReturnType(type)).flatMap((receipt, index, filtered) => {
    ...
    if (receipt.type === import_transactions4.ReceiptType.Return) {
      return [
        new import_abi_coder2.BigNumberCoder("u64").encode(receipt.val)
      ];
    }
...
var BaseInvocationScope = class {
  ...
  async prepareTransaction() {
    ...
    if (this.transactionRequest.type === import_transactions6.TransactionType.Script) {
  ...
  async simulate() {
    ...
    if (!("populateTransactionWitnessesSignature" in this.program.account)) {
      throw new import_errors5.FuelError(
        import_errors5.ErrorCode.ABI_MAIN_METHOD_MISSING,
